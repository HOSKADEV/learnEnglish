...
interface Question {
  id?: string;
  sentence: string;        // الجملة مع ___ في مكان الفراغ
  blank: string;           // الكلمة الصحيحة
  options: string[];       // الخيارات (3 أو 4)
  translation: string;     // الترجمة العربية للجملة
  order: number;
}
...
export default function ManageFillBlank() {
  ...
  const save = async () => {
    if (!form.sentence.includes("___") || !form.blank.trim() || !form.translation.trim()) {
      return alert("تأكد من وجود ___ في الجملة وملء كل الحقول");
    }

    const options = [form.option1, form.option2, form.option3].filter(Boolean);
    options.push(form.blank); // نضيف الإجابة الصحيحة
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4">
      ...
      {showAddEditModal && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999}}
          ...
          <div style={{background:"white",padding:"40px",borderRadius:"20px",width:"90%",maxWidth:"700px",position:"relative"}}
            ...
            <textarea
              ...
              autoFocus
            />
            <input placeholder="الإجابة الصحيحة" value={form.blank}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              ...
              <input placeholder="خيار 2" value={form.option2}
                ...
                className="w-full p-4 border-2 border-gray-300 rounded-xl text-lg focus:border-green-500 outline-none" />
              <input placeholder="خيار 3 (اختياري)" value={form.option3}
                onChange={e => setForm({...form, option3: e.target.value})}
                className="w-full p-4 border-2 border-gray-300 rounded-xl text-lg focus:border-green-500 outline-none" />
            </div>
            <input placeholder="الترجمة العربية للجملة" value={form.translation}
